...
class Transformer(nn.Module):
    ...
    def forward(self, x_c, y_c,x_s,y_s,x_m,y_m,x_parse,y_parse):
        x_f = self.fusion(x_c,x_s)
        y_f = self.fusion(y_c, y_s)
        attention_x,attention_y,x_m_warp,y_m_warp=self.atte(x_f,y_f,x_m,y_m,x_parse,y_parse)
...
class SymmetryAttention(nn.Module):
    ...
    def warp(self, fa, fb, a_raw, b_raw,a_mask,b_mask, alpha):
        '''
        ...
        a_mask = a_mask.view(n, mask_c, -1)
        b_mask = b_mask.view(n, mask_c, -1)
        ...
        mask_ab_T = torch.bmm(b_mask.transpose(-2, -1), a_mask)
        ...
        mask_ba_T = torch.bmm(a_mask.transpose(-2, -1), b_mask)

        energy_ba_T = energy_ba_T * mask_ba_T
    ...
    def forward(self, fa, fb, a_raw, b_raw,x_parse,y_parse):
        fa = self.fa_conv(fa)
        fb = self.fb_conv(fb)
        X, Y, a_warp, b_warp = self.warp(fa, fb, a_raw, b_raw, x_parse,y_parse,self.softmax_alpha)
...
class Decoder(nn.Module):
    def __init__(self, output_dim=3, ngf=64):
        ...
        self.residual_layer = self.make_layer(Residule_Block, 4, ngf * 8)
    ...
    def forward(self, x, y):
        ...
        makeup = y
        # out = content+makeup
        out = torch.cat([content,makeup],dim=1)
        # x (ngf*8, 64, 64)
...
class DecoderSPADE(nn.Module):
    def __init__(self, output_dim=3, ngf=64):
        super(DecoderSPADE, self).__init__()
