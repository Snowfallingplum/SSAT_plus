...
class Saver():
    ...
    # save test inter images
    def write_test_inter_img(self, iter, model,data):
        root = os.path.join(self.result_dir, 'test_inter')
        if not os.path.exists(root):
            os.makedirs(root)
        test_pair_img = model.test_inter1(data)
        img_filename = '%s/gen_%05d.jpg' % (root, iter)
        torchvision.utils.save_image(test_pair_img / 2 + 0.5, img_filename, nrow=1)

    # save test inter2 images
    def write_test_inter2_img(self, iter, model, data):
        root = os.path.join(self.result_dir, 'test_inter2')
        if not os.path.exists(root):
            os.makedirs(root)
        test_pair_img = model.test_inter2(data)
        img_filename = '%s/gen_%05d.jpg' % (root, iter)
        torchvision.utils.save_image(test_pair_img / 2 + 0.5, img_filename, nrow=1)

    # save test mix images
    def write_test_mix_img(self, iter, model, data):
        root = os.path.join(self.result_dir, 'test_mix_lip')
        if not os.path.exists(root):
            os.makedirs(root)
        test_pair_img = model.test_mix(data)
        img_filename = '%s/gen_%05d.jpg' % (root, iter)
        torchvision.utils.save_image(test_pair_img / 2 + 0.5, img_filename, nrow=1)
