...
class MakeupDataset(data.Dataset):
    def __init__(self, opts):
        ...
        print('dataset:',self.dataset_size)
    ...
    def load_mask(self,path,angle=0):
        ...
        face_mask[np.where(parse == 9)] = 0
        face_mask[np.where(parse == 4)] = 0
        face_mask[np.where(parse == 5)] = 0
        face_mask[np.where(parse == 11)] = 0
    ...
    def local_masks(self, split_parse):
        ...
        face_mask[np.where(split_parse[:, :, 9] == 1)] = 0# qu diao bo zi
    ...
    def __getitem__(self, index):
        if self.opt.phase == 'train':
            ...
            non_makeup_mask = self.load_img(self.non_makeup_path[index].replace('images', 'seg1')[:-4]+'.png', non_makeup_angle)
            non_makeup_parse = self.load_parse(self.non_makeup_path[index].replace('images', 'seg1')[:-4]+'.png', non_makeup_angle)

            # load makeup
            index_other = random.randint(0, self.makeup_size - 1)
            makeup_img = self.load_img(self.makeup_path[index_other], makeup_angle)
            makeup_mask = self.load_img(self.makeup_path[index_other].replace('images', 'seg1')[:-4]+'.png', makeup_angle)
            makeup_parse = self.load_parse(self.makeup_path[index_other].replace('images', 'seg1')[:-4]+'.png', makeup_angle)
            ...
            non_makeup_parse = self.load_parse(self.non_makeup_path[non_makeup_index].replace('images', 'seg1')[:-4]+'.png',non_makeup_angle)
...
if __name__ == '__main__':
    ...
    opts = parser.parse()
    # C:\Users\zhaoy\Desktop\SSAT_code\makeup_datasets\images
    opts.dataroot = '../../SSAT_code/makeup_datasets/images/'
    # data loader
    print('\n--- load dataset ---')
    ...
    for i in range(3):
        ...
        imgs = [data['non_makeup'], data['makeup'], data['transfer'], data['removal'],
                data['non_makeup_all_mask'],data['non_makeup_face_mask'],data['non_makeup_brow_mask'],data['non_makeup_eye_mask'],data['non_makeup_lip_mask'],
                data['makeup_all_mask'],data['makeup_face_mask'],data['makeup_brow_mask'],data['makeup_eye_mask'],data['makeup_lip_mask']]
        names = [str(i) + '_non_makeup.jpg', str(i) + '_makeup.jpg', str(i) + '_transfer.jpg', str(i) + '_removal.jpg',
                 str(i)+'non_makeup_all_mask.jpg',str(i)+'non_makeup_face_mask.jpg',str(i)+'non_makeup_brow_mask.jpg',
                 str(i)+'non_makeup_eye_mask.jpg',str(i)+'non_makeup_lip_mask.jpg',
                 str(i) + 'makeup_all_mask.jpg', str(i) + 'makeup_face_mask.jpg', str(i) + 'makeup_brow_mask.jpg',
                 str(i) + 'makeup_eye_mask.jpg', str(i) + 'makeup_lip_mask.jpg']
